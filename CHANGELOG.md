...
- **Navigation Guard Hardened**: Removed insecure `.endsWith("index.html")` fallback in `will-navigate` to only allow strict equality against the app's renderer HTML.
- **SFTP IPC Sender Verification**: Added `isTrustedSender(event)` checks to all SFTP handlers to reject untrusted webview guests.
- **Ports & Connections IPC Sender Verification**: Added `isTrustedSender(event)` to port and connection handlers to prevent untrusted webviews from enumerating or killing processes.

### Fixed

- **SQLite Pruning Crash Fix**: Replaced dynamic `IN (?, ?, ...)` clause with a SQL subquery in `historyDb.ts` for pruning history.
- **Atomic Database Operations**: Wrapped `deleteSession` and `clearHistory` in `db.transaction()` to prevent orphaned FTS records on crash.
- **PTY Cleanup Fix**: Wrapped `destroyTerminal` map cleanup in a `try/finally` block to ensure it always runs.
- **Zustand Selectors Optimization**: Refactored `ClipboardHistoryPanel` and `TitleBar` to use granular Zustand selectors, preventing unnecessary re-renders.
- **Hash Fragment Sanitization**: Improved file path sanitization by replacing `split('#')[0]` with `indexOf('#')` in `workspace.ts` and added hash stripping to the `get-git-diff` handler.

...
export function initClipboardHistoryManager() {
  ...
  ipcMain.handle('clipboard:set-history', async (_event, items: ClipboardItem[]) => {
...
async function loadHistory(): Promise<ClipboardItem[]> {
  try {
    ...
    const keepDays = typeof config.clipboardHistoryKeepDays === 'number' ? config.clipboardHistoryKeepDays : 7

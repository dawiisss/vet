...
async function getSftpSession(sshHostId: string): Promise<SftpSession> {
  ...
  if (!hostConfig && sshHostId.startsWith('auto-')) {
    ...
    const matchingProfile = config.sshHosts?.find((h: any) => h.host === host && (!target.includes('@') || h.username === user))
    if (matchingProfile) {
  ...
  if (hostConfig.authType === 'password') {
    if (!hostConfig.password) throw new Error('Password required but not provided in profile')
    connOpts.password = hostConfig.password
  } else if (hostConfig.authType === 'key') {
    if (!hostConfig.privateKeyPath) throw new Error('Private key path required')
...
export function initSftpManager() {
  ...
  ipcMain.handle('sftp:read-file-head', async (_, sshHostId: string, filePath: string) => {
    try {
      const session = await getSftpSession(sshHostId)
      return new Promise((resolve, reject) => {
        ...
        session.sftp.open(targetPath, 'r', (err, handle) => {
          ...
          session.sftp.read(handle, buffer, 0, buffer.length, 0, (err, bytesRead) => {

...
export type UpdateStatus = 'idle' | 'checking' | 'available' | 'uptodate' | 'downloading' | 'downloaded' | 'error'
...
export const useUpdaterStore = create<UpdaterState>((set, get) => ({
  ...
  init: () => {
    ...
    if (!api) {
      console.warn('Updater API is not available on window. Make sure you are running in Electron.')
      return () => {}
    }
    ...
    const unsubStatus = api.onStatusChange((status, info) => {
      ...
      if (status === 'available' || status === 'downloaded') {
        ...
        set({ error: typeof info === 'string' ? info : 'An unknown updater error occurred.' })

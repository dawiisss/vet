...
export const UpdateModal: React.FC<UpdateModalProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    if (window.windowApi?.getVersion) {
      window.windowApi.getVersion().then(setCurrentVersion).catch(console.error)
    }
  }, [])
      ...
      >
        ...
        <div
          style={{
            ...
            height: 60,
            borderRadius: 16,
            background: 'linear-gradient(135deg, var(--app-accent, #10b981) 0%, color-mix(in srgb, var(--app-accent, #10b981) 60%, #000) 100%)',
            boxShadow: '0 8px 24px -4px rgba(16, 185, 129, 0.2), 0 0 16px rgba(16, 185, 129, 0.1)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            marginBottom: 16
        ...
        >
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--app-bg, #000)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 12, fontSize: 13, color: 'var(--app-fg-subtle)' }}>
          <div>Current: <strong style={{ color: 'var(--app-fg)' }}>{currentVersion}</strong></div>
          <div style={{ color: 'rgba(255,255,255,0.15)' }}>|</div>
          <div>New: <strong style={{ color: 'var(--app-accent, #10b981)' }}>{updateInfo?.version || 'Unknown'}</strong></div>
        </div>

        {/* Divider */}
        <div style={{ width: '100%', height: 1, backgroundColor: 'rgba(255, 255, 255, 0.08)', margin: '18px 0' }} />

        {/* Update Notes / Release Info */}
        {updateInfo?.releaseNotes && (
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 20 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--app-fg-muted)', alignSelf: 'flex-start' }}>Release Notes</span>
            <div className="vet-update-notes">
              {updateInfo.releaseNotes}
            </div>
          </div>
        )}
        ...
        {status === 'downloading' && progress && (
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--app-fg-muted)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--app-fg-muted)', marginTop: 2 }}>
        ...
        {status === 'downloaded' && (
          <div style={{ color: 'var(--app-accent, #10b981)', fontSize: 13, fontWeight: 500, marginBottom: 20 }}>
            ✓ Update downloaded and ready to install.
          </div>
        )}

        {error && (
          <div style={{ color: '#ef4444', fontSize: 13, fontWeight: 500, marginBottom: 20, textAlign: 'center' }}>
            ⚠ {error}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', width: '100%' }}>
          {status !== 'downloading' && (
            <button
              className="vet-update-btn-secondary"
              onClick={onClose}
            >
              Cancel
            </button>
          ...
          {status === 'available' && (
            <button
              className="vet-update-btn-primary"

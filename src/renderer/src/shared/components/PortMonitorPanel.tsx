...
export default function PortMonitorPanel({ isActive }: { isActive: boolean }) {
  ...
  return (
    <Panel title="Listening Ports" headerActions={headerActions}>
      {ports.length === 0 && !loading && (
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: 12 }}>
            Active processes will appear here
          </p>
        </div>
      )}
      {ports.map((p) => (
        ...
        >
          <div>
            <div style={{ fontWeight: "bold", color: "#89dceb" }}>
              :{p.port}
            </div>
            <div style={{ fontSize: 11, color: "var(--app-fg-subtle)" }}>
              {p.process} (PID: {p.pid})
            </div>
          </div>
          <button

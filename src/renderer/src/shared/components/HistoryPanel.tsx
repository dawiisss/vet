...
export default function HistoryPanel({
  ...
  const [activeTab, setActiveTab] = useState<"terminal" | "browser">(
    "terminal",
  );
  ...
  const handleClear = async () => {
    if (activeTab === "terminal") {
      if (
        window.confirm("Are you sure you want to clear all terminal history?")
      ) {
        try {
      ...
      if (
        window.confirm("Are you sure you want to clear all browser history?")
      ) {
        try {
    ...
    <Panel hasScrollableBody={false} style={{ padding: 0 }}>
      ...
      >
        ...
        <input
          ...
          placeholder={
            activeTab === "terminal"
              ? "Search commands or output..."
              : "Search URLs or page titles..."
          }
          style={{
            width: "100%",
            background: "rgba(0,0,0,0.2)",
                ...
                >
                  ...
                  >
                    <span style={{ fontSize: 12 }}>
                      {session.connection_type === "ssh"
                        ? "🌐"
                        : session.connection_type === "docker"
                          ? "🐳"
                          : "💻"}
                    </span>
                    <span
                ...
                {session.snippet && (
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: DOMPurify.sanitize(session.snippet, {
                        ALLOWED_TAGS: ["b"],
                        ALLOWED_ATTR: [],
                      }),
                    }}

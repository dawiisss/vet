...
export const BrowserView: React.FC<BrowserViewProps> = ({
  ...
  const [localAdblockEnabled, setLocalAdblockEnabled] =
    useState(isAdblockEnabled);
  const [appPreloadPath, setAppPreloadPath] = useState<string | undefined>(
    undefined,
  );
  ...
  const searchShortcut =
    Object.entries(config.keybindings || {}).find(
    ...
    const onIpcMessage = (e: any) => {
      ...
      try {
        ...
        document.dispatchEvent(
          new MouseEvent(type === "move" ? "mousemove" : "mouseup", {
            clientX: rect.left + x,
            clientY: rect.top + y,
            button: button ?? 0,
            bubbles: true,
    ...
    >
      ...
      >
        ...
        <button
          ...
          title={
            localAdblockEnabled
              ? `Adblocker Enabled: ${blockedCount} blocked`
              : "Adblocker Disabled"
          }
          style={{
            background: localAdblockEnabled
              ? "rgba(235, 111, 146, 0.15)"
              : "rgba(255, 255, 255, 0.05)",
            border: `1px solid ${localAdblockEnabled ? "var(--app-red)" : "var(--app-border)"}`,
            borderRadius: 6,
            color: localAdblockEnabled
              ? "var(--app-red)"
              : "var(--app-fg-muted)",
            padding: "4px 8px",
            fontSize: 11,

...
export const ThemeEditor: React.FC = () => {
  ...
  if (editingThemeId && draftTheme) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, height: 350, overflowY: 'auto', paddingRight: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, fontSize: 16 }}>{editingThemeId === 'new' ? 'Create Custom Theme' : 'Edit Theme'}</h3>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          ...
          {/* Terminal Preview */}
          <div style={{ flex: 1, background: draftTheme.background, border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: 12, fontFamily: 'monospace', fontSize: 12, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ color: draftTheme.green }}>➜ <span style={{ color: draftTheme.cyan }}>~</span> <span style={{ color: draftTheme.foreground }}>ls -la</span></div>
            ...
            <div style={{ marginTop: 8, display: 'flex', gap: 2 }}>
              {ANSI_COLORS.slice(0,8).map(c => <div key={c} style={{ width: 12, height: 12, background: draftTheme[c], borderRadius: 2 }} />)}
            </div>
            <div style={{ display: 'flex', gap: 2 }}>
              {ANSI_COLORS.slice(8,16).map(c => <div key={c} style={{ width: 12, height: 12, background: draftTheme[c], borderRadius: 2 }} />)}
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: 350, overflowY: 'auto', paddingRight: 8 }}>
      <button
        ...
        return (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {isCustom && <span style={{ fontSize: 10, background: 'rgba(255,255,255,0.1)', padding: '2px 6px', borderRadius: 10 }}>Custom</span>}
            </div>
            
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <div style={{ display: 'flex', gap: 4 }}>
                <div style={{ width: 12, height: 12, borderRadius: '50%', background: t.background }} />
                <div style={{ width: 12, height: 12, borderRadius: '50%', background: t.foreground }} />

...
export default function WorkspacePanel({
  ...
  useEffect(() => {
    ...
    const checkCwd = async () => {
      ...
      try {
        const info = await window.terminalApi.getTerminalInfo(
  ...
  const getFileIcon = (item: WorkspaceItem) => {
    ...
    if ([".sh", ".bash", ".zsh", ".py", ".go", ".rs"].includes(ext))
      return "🐚";
    return "📄";
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (target.isDirectory) {
        ...
        const fullPath =
          cwd === "/" ? `/${target.name}` : `${cwd}/${target.name}`;
    ...
    >
      ...
      {!activeTerminalId ? (
        <div style={{ color: "var(--app-fg-muted)", fontStyle: "italic" }}>
          Open a terminal tab to view files
        </div>
      ) : (
        <>
          {/* CWD Path display */}
          ...
          >
            <span style={{ color: "var(--app-blue)", flexShrink: 0 }}>
              cwd:
            </span>
            <span
              style={{
          ...
          >
            {authRequired ? (
              ...
              >
                <div style={{ color: "var(--app-red)", fontWeight: "bold" }}>
                  Authentication Required
                </div>
                <div
                  style={{ color: "#bac2de", fontSize: 12, marginBottom: 8 }}
                >
                  Please enter the SSH password for this session:
                </div>
                <input
                  type="password"
                  value={tempPassword}
                  ...
                  onKeyDown={async (e) => {
                    if (e.key === "Enter" && sshHostId && tempPassword) {
                      await window.sftpApi.setTempPassword(
                        sshHostId,
                        tempPassword,
                      );
                      setTempPassword("");

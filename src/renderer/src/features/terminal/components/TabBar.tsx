...
function TabBar({
  ...
  const handleTabMouseDown = useCallback(
    (tabId: string) => (e: React.MouseEvent) => {
      ...
      ghost.textContent =
        tabsRef.current.find((t) => t.id === tabId)?.label ?? "shell";
      ghost.style.cssText = `
        position: fixed;
        pointer-events: none;
        z-index: 9999;
              ...
              >
                {editingTabId === tab.id ? (
                  <input
                    ...
                    onBlur={() => {
                      if (onRenameTab && editingLabel.trim())
                        onRenameTab(tab.id, editingLabel.trim());
            ...
            <>
              ...
              <div
                style={{
                  ...
                  right: 0,
                  background:
                    "color-mix(in srgb, var(--app-bg) 95%, transparent)",

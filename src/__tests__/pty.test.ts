...
describe('pty', () => {
  ...
  describe('createTerminal', () => {
    ...
    it('spawns a pty process with expected shell', () => {
      expect(spawn).toHaveBeenCalledWith('/bin/bash', [], expect.objectContaining({
        name: 'xterm-256color',
        cols: 80,
        rows: 24,
      }))
    ...
    it('detects SSH connections by shell name', () => {
      const config = require('../main/config')
      config.getConfig.mockReturnValue({
        profiles: [{ id: 'ssh-profile', name: 'SSH', shell: 'ssh', args: ['user@host'] }],
      })
    ...
    it('registers onData handler that saves to history buffer', () => {
      const mockPty = (spawn as jest.Mock).mock.results[spawn.mock.results.length - 1].value

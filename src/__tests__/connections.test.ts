...
function mockExecCallback(stdout: string, error: Error | null = null) {
  execFileMock.mockImplementation(
    (_file: string, _args: string[], cb: (...args: unknown[]) => void) => {
...
describe("connections", () => {
  ...
  beforeEach(() => {
    ...
    sshHandler = calls.find(
      (c: string[]) => c[0] === "connections:get-ssh-hosts",
    )?.[1];
    dockerHandler = calls.find(
      (c: string[]) => c[0] === "connections:get-docker",
    )?.[1];
  });
